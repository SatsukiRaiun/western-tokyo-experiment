...
現在、このタグを使っているファイルは[こちら](https://github.com/search?q=repo%3Apetelomax%2FPhix+<col&type=code&p=1)です。ほとんどの場合は、削除すれば通るようにはなりますが、[Technicalia](https://github.com/search?q=repo%3Apetelomax%2FPhix path%3A%2F%5Edocs%5C%2Fphix%5C%2Fsrc%5C%2F%2FTechnicalia&type=code) などの変換で失敗したり、テーブルは残ったままになるので考えものです(対策は後ほど)。このタグ以外にも追加修正しないと正常に変換できないコトがあります。Pandoc変換後は必ず内容の照合をしましょう。
